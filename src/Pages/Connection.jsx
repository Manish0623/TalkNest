      ...
      <div className='max-w-6xl mx-auto p-6'>
        ...
        <div className='mb-8 flex flex-wrap gap-6'>
          ...
          rounded-md p-1 bg-white shadow-sm'>
            {
              dataArray.map((tab)=>(
                <button onClick={(()=> setCurrentTab(tab.label))} key={tab.label} className={` cursor-pointer 
...
{dataArray.find((item)=> item.label === currentTab).value.map((user)=>(
  <div key={user._id} className='w-full max-w-88 flex gap-5 p-6 bg-white shadow rounded-md'>
    <img className='rounded-full w-12 h-12 shadow-md mx-auto' src={user.profile_picture}/>
    <div className='flex-1'>
<p className='font-medium text-slate-700'>{user.full_name}</p>
<p className=' text-slate-700'>{user.username}</p>
<p className=' text-sm text-slate-500'> {user.bio.slice(0 , 30)}...</p>
<div className='flex max-sm:fkex-col gap-2 mt-4'>
  {
    <button onClick={()=> navigate(`/profile/${user._id}`)} className='w-full p-2 text-sm rounded bg-gradient-to-r from-blue-600
     to-blue-700 hover:from-blue-700 hover:to-blue-900 active:scale-100
